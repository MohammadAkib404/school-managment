    ...
    <nav className="flex flex-col h-full" aria-label="Main navigation">
      ...
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-border shrink-0">
        ...
        <div className="flex flex-col gap-px min-w-0">
          ...
          <span className="text-xs text-muted">Management Portal</span>
        </div>
        {/* Close button — mobile only */}
        <button
          onClick={() => setOpen(false)}
          aria-label="Close menu"
          className="md:hidden ml-auto w-7 h-7 rounded-lg bg-surface text-muted text-sm flex items-center justify-center hover:bg-border hover:text-primary transition-colors"
        >
          ✕
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1.5 scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
        {NAV.map((section) => (
          <div key={section.title} className="mb-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted px-2 pt-3 pb-1">{section.title}</p>
      ...
      <div className="flex items-center gap-2.5 px-3.5 py-3 border-t border-border bg-surface shrink-0">
        ...
        <button
          aria-label="Sign out"
          className="w-7 h-7 rounded-lg border border-border bg-base text-muted text-sm flex items-center justify-center hover:bg-danger/10 hover:text-danger hover:border-danger/30 transition-colors"
        >
          ↩
        </button>
      </div>
